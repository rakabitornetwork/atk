            ...
            <Card className="max-w-2xl">
                <form onSubmit={(e) => { e.preventDefault(); post('/settings', { forceFormData: true }); }} className="grid gap-2 sm:grid-cols-2">
                    <FieldLabel label="Logo aplikasi" help="Logo utama aplikasi untuk branding. Cocok untuk gambar horizontal atau logo lengkap toko." className="sm:col-span-2">
                        {settings?.logo_url ? <img src={settings.logo_url} alt="Logo aplikasi" onError={(event) => { event.currentTarget.style.display = 'none'; }} className="mb-2 h-12 rounded border border-[var(--atk-border)] bg-white object-contain p-1" /> : null}
                    ...
                    <FieldLabel label="Logo icon" help="Icon kecil aplikasi. Disarankan bentuk persegi, dipakai sebagai pengganti kotak ATK jika tersedia.">
                        {settings?.logo_icon_url ? <img src={settings.logo_icon_url} alt="Logo icon" onError={(event) => { event.currentTarget.style.display = 'none'; }} className="mb-2 h-10 w-10 rounded-lg border border-[var(--atk-border)] bg-white object-contain p-1" /> : null}
                    ...
                    <FieldLabel label="Logo navbar" help="Logo yang tampil di bagian branding sidebar/navbar. Jika kosong, sistem memakai nama toko dan icon default.">
                        {settings?.navbar_logo_url ? <img src={settings.navbar_logo_url} alt="Logo navbar" onError={(event) => { event.currentTarget.style.display = 'none'; }} className="mb-2 h-10 rounded border border-[var(--atk-border)] bg-white object-contain p-1" /> : null}
